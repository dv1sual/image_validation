...
def load_image(image_path):
    ...
    if image_path.lower().endswith(".exr"):
        ...
        image = np.array(Image.open(image_path)) / 255.0  # Convert the non-EXR images to floating-point values in the 0-1 range
...
def validate_color_chart(image, chart_values):
    ...
    for color_patch, accuracy in results:
        if accuracy < 99.99:
            print(colored(
                "[CHECK] - Color patch {} failed validation with accuracy {:.2f}%".format(color_patch, accuracy),
                "red"))
        else:
            print(colored(
                "[CHECK] - Color patch {} passed validation with accuracy {:.2f}%".format(color_patch, accuracy),
                "green"))
